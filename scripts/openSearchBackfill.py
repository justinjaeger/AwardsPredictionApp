...
# see note "opensearch backfill script"
...
def main():
  ...
  parser.add_argument('--lf', metavar='LF', help='lambda function that posts data to es - Lambda->Functions', required=True) 
  parser.add_argument('--esarn', metavar='ESARN', help='event source ARN - DynamoDB->Tables->table_name->exports and streams->stream details', required=True) 
  parser.add_argument('--ak', metavar='AK', help='aws access key') # can generate new in IAM. Create new user role and copy access
  parser.add_argument('--sk', metavar='AS', help='aws secret key') # can generate new in IAM. Create new user role and copy access
  parser.add_argument('--st', metavar='AT', help='aws session token') # unnecessary
